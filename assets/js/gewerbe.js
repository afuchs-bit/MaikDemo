...
// AP-109 – Gewerbe-Leistungskarten: zugängliche Flip-Interaktion mit nur einer
// offenen Karte. 1:1-Port der Privatkunden-IIFE aus privat-form.js, nur der
// Sektions-Scope unterscheidet sich (.gewerbe-request-paths).
(() => {
  'use strict';

  const section = document.querySelector('.gewerbe-request-paths');
  ...
  const scheduleFocus = (card, target) => {
    window.clearTimeout(card._gewerbeFlipFocusTimer);
    card._gewerbeFlipFocusTimer = window.setTimeout(() => {
      target?.focus({ preventScroll: true });
