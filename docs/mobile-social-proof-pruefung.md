...
## AP-212 – Zwei Bild-Einbindungen im Willkommenbereich (05.09.2026)

- Varianten werden ausschließlich über `?bildvorschau=flaechig` und
  `?bildvorschau=fenster` aktiviert; der Aufruf ohne Parameter bleibt unverändert.
- Beide Varianten verwenden unverändert das vorhandene responsive Poolgarten-
  Asset und verändern keine Texte oder Qualifikationen.
- Geltungsbereich auf 390–404 px im Hochformat begrenzt; Zielprüfung bei
  393 × 852 und 402 × 874 CSS-Pixeln.
- Eigene CSS-Datei mit Cachekennung `20260905a`; gemeinsamer Sprunganker
  `#willkommen-bild` für den direkten Vergleich.
- Bei 393 × 852 und 402 × 874 CSS-Pixeln laden beide Motive ohne Konsolen- oder
  Netzwerkfehler. Die flächige Variante nutzt eine 430 px hohe Bildbühne; das
  Fenster misst vor seiner leichten Neigung 354 × 272 px beziehungsweise
  363 × 272 px.
- Das Foto bleibt in beiden Varianten proportional und wird mit `object-fit:
  cover` beschnitten. Die flächige Variante zeigt Kicker und Überschrift auf dem
  Bild mit getrennten Lesbarkeitsverläufen; das Bildfenster besitzt links 52 px
  Radius, eine 2 px grüne Kontur und 3 Grad Neigung.
- Dokument- und Ansichtsbreite zeigen bei beiden Zielgrößen keinen horizontalen
  Seitenüberstand. Ohne Vorschauparameter bleibt das Bestandsfoto bei 393 px
  unverändert 330 × 247,5 px groß, mit 18 px Radius und ohne Transformation.
